...
pub struct FreezeNft<'info> {
    ...
    #[account(mut, constraint = !asset.data_is_empty() @ MPLXCoreError::CollectionNotInitialized)]
...
impl<'info> FreezeNft<'info> {
    pub fn freeze_nft(&mut self) -> Result<()> {
        require!(self.authority.key() == self.collection_authority.creator, MPLXCoreError::NotAuthorized);
        let signer_seeds: &[&[&[u8]]] = &[&[
